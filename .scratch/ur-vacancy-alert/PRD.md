...
1. 所属**団地**在**通勤候选団地(watchlist)**内 —— 即到**公司地址 東京都中央区晴海1-8-10(晴海トリトンスクエア)门到门 ≤ 70 分**的神奈川団地。目标是地址而非单一车站(周边有勝どき/月島/新豊洲/晴海BRT 多个接入口,取最快)。阈值演变(≤40勝どき站→≤55→地址≤70)及全 166 扫描理由见 `docs/adr/0003-commute-threshold-55min.md` 与 `data/commute_to_katsudoki.md`。
...
2. ~~`02` 半手动生成 watchlist~~ ✅ **完成(2026-06-23 全量重做)**:目标改为公司地址晴海1-8-10(门到门 ≤70 分,考虑勝どき/月島/BRT 等多接入口),在**全 166 個神奈川団地**上扫描、各站→勝どき 逐一拉 Yahoo!路線情報实际路线页核实 → 筛出 **31 個団地** → `watchlist.json`(每項带 `commute_min`=到办公室分钟)。演变 ≤40站→≤55→地址≤70、及搜索摘要曾致偏差的教训见 ADR-0003。
